...
- (id)initWithStyle:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        [self setTitle:@"Blog"];
        [self.tabBarItem setImage:[UIImage imageNamed:@"iconBlog.png"]];
        [self getPosts];
...
-(void) getPosts
{
    ...
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        NSString *url = [NSString stringWithFormat:@"%@?action=nosmidia_api&post_type=post&posts_per_page=100", API_BLOG_URL];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if( [_data count] >0 ){
        ...
        if(isLoading)
            [cell.textLabel setText:@"Carregando posts..."];
        else
            [cell.textLabel setText:@"Nenhum post encontrado."];
        
    }
    
   
    [cell.imageView setImage:[UIImage imageNamed:@"iconBlog.png"]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *urlAddress =[NSString stringWithFormat:@"%@?action=nosmidia_api_single&post_type=post&posts_per_page=1&include=%d", API_BLOG_URL,postId];
